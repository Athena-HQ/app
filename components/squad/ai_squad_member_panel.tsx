...
import { Sparkles, User, Star, AlertTriangle, CheckCircle2, Clock } from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import type { AISquadMemberSuggestion } from "@/hooks/useAISquadMemberSuggestions";

function ConfidencePip({ confidence }: { confidence: AISquadMemberSuggestion["confidence"] }) {
  const map: Record<AISquadMemberSuggestion["confidence"], { label: string; cls: string }> = {
...
interface AISquadMemberSuggestionsPanelProps {
...
export function AISquadMemberSuggestionsPanel({
  suggestions,
  loading,
  error,
  onTrigger,
  onPick,
  disabled,
}: AISquadMemberSuggestionsPanelProps) {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            id="ai-suggest-member-btn"
          >
            <Sparkles className="h-4 w-4" />
            {loading ? "Analyzing engineers…" : "Suggest Member with AI"}
          </Button>
          {disabled && !loading && (
            <p className="text-xs text-muted-foreground">
              Fill in Squad Stack and Mission first
            </p>
          )}
        </div>
            ...
            {suggestions.map((s, idx) => (
              ...
              >
                  ...
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {s.is_busy && (
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{s.open_task_count} open tasks currently</p>
                        </TooltipContent>
                      </Tooltip>
                    )}
                  </div>
                ...
                <Button
                  ...
                  id={`ai-pick-member-${s.employee_id}`}
                >
                  Pick
                </Button>
              </div>
